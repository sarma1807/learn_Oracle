...
if [[ ${NO_OF_NET_INTERFACES} -eq 0 ]]; then
  echo "ERROR : could not find 'eth*' network interfaces" >&2
  exit 1
fi
...
for input_arg in "$@"; do
  case "${input_arg}" in
    ...
    *)
      echo "Please provide full hostname for this host and make sure this entry has beed added in /etc/hosts file." >&2
      echo "Usage : $0 --full_host_name=<hostname> [ --ignore_private_interface ]" >&2
      exit 1
      ;;
  esac
done
...
if [[ ${HOST_FILE_ENTRY_COUNT} -eq 0 ]]; then
  echo "ERROR : could not find '${INPUT_HOST_NAME}' in /etc/hosts file" >&2
  exit 1
fi

# error if provided input hostname entry is found multiple times in /etc/hosts file
if [[ ${HOST_FILE_ENTRY_COUNT} -gt 1 ]]; then
  echo "ERROR : found multiple entries matching '${INPUT_HOST_NAME}' in /etc/hosts file" >&2
  exit 1
fi
...
if ! ${IGNORE_PRIVATE_INTERFACE}; then
  ...
  if [[ ${PRIV_HOST_FILE_ENTRY_COUNT} -eq 0 ]]; then
    echo "ERROR : could not find '${HPN}' in /etc/hosts file" >&2
    exit 1
  fi
  ...
  if [[ ${PRIV_HOST_FILE_ENTRY_COUNT} -gt 1 ]]; then
    echo "ERROR : found multiple entries matching '${HPN}' in /etc/hosts file" >&2
    exit 1
  fi
fi
