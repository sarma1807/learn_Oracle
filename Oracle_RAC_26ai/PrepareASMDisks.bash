...
if [[ ${COUNT_OF_DISKS_FOR_ASM} -eq 0 ]]; then
  ...
  echo "ERROR : could not find any disks listed for Oracle ASM usage in above listed 'ASM Disks File' file"
  echo "USAGE :"
  echo "- add entries for disks, which you want to use for Oracle ASM in above listed 'ASM Disks File' file"
  echo "- add one disk per line entry"
  echo "EXAMPLES OF VALID ENTRIES :"
  echo "/dev/sdb"
  echo "/dev/sdc"
  echo "/dev/sdd"
  echo "DO NOT ADD PARTITION NAMES, SUCH AS FOLLOWING :"
  echo "/dev/sdx1"
  echo "/dev/sdx2"
  echo "/dev/sdy1"
  exit 1
...
if [[ ${LIST_OF_VALID_ASM_DISKS_FINAL_COUNT} -eq 0 ]]; then
  echo "ERROR : could not identify any disks for Oracle ASM usage"
